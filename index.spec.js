const ContractEventListener = require('./index');
...
describe('ContractEventListener', function() {
  describe('constructor' , () => {
    describe('when no parameters are passed', () => {
      it('should throw an error', function() {
        expect(() =>  new ContractEventListener()).toThrow('No web3 instance provided.');
      });
    });

    describe('when no address is passed', () => {
      it('should throw an error', function() {
        expect(() =>  new ContractEventListener(web3mock)).toThrow('No contract address provided.');
      });
    });

    describe('when contract abi is not passed', () => {
      it('should throw an error', function() {
        expect(() =>  new ContractEventListener(web3mock, "0x0")).toThrow('No contract abi provided.');
      });
    });

    describe('when all parameters are passed', () => {
      it('should return a ContractEventListener instance', function() {
        const eventListener = new ContractEventListener(web3mock, "0x0", { foo: 'bar' });
  ...
  describe('#on()', () => {
    web3mock.eth.getBlockNumber.mockReturnValue(1);
    ...
    describe('when listening for the "end" event', () => {
      ...
      it('should assign a callback to call when new events has finished processing', async () => {
        ...
        await sleep(1);
        eventListener.stop();
        expect(eventCallback).toHaveBeenCalledWith(1, 1);
      });
    });
